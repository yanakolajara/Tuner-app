...
export function Songs() {
  ...
  function sortAbcCheck(x) {
    ...
    if (queryParams.get('order') === 'asc') {
      ...
      if (queryParams.get('is_favorite') === 'true') {
        navigate('/songs?order=asc&is_favorite=true');
      } else if (queryParams.get('is_favorite') === 'false') {
        navigate('/songs?order=asc&is_favorite=false');
      } else {
  ...
  function sortFavCheck(x) {
    ...
    if (queryParams.get('is_favorite') === 'true') {
      if (queryParams.get('order') === 'asc') {
        ...
        navigate('/songs?is_favorite=false');
      }
    } else if (
      queryParams.get('is_favorite') === 'false' ||
      queryParams.get('is_favorite') === null
    ) {
      if (queryParams.get('order') === 'asc') {
        navigate('/songs?order=asc&is_favorite=true');
      } else if (queryParams.get('order') === 'desc') {
  ...
  function displaySongs() {
    ...
    if (queryParams.get('order') === 'asc') {
      const ascArr = songsArr.sort((a, b) =>
        a.props.children[0].props.name < b.props.children[0].props.name ? -1 : 1
      );
      finalArr = ascArr;
    } else if (queryParams.get('order') === 'desc') {
      const descArr = songsArr.sort((a, b) =>
        a.props.children[0].props.name > b.props.children[0].props.name ? -1 : 1
      );
      finalArr = descArr;
    }
    if (queryParams.get('is_favorite') === 'true') {
      finalArr = finalArr.filter(
        (x) => !!x.props.children[0].props.is_favorite
      );
    } else if (queryParams.get('is_favorite') === 'false') {
  ...
  return (
    <main>
      <div class='sortButtons'>
        <button class='sortAbcButton' onClick={(x) => sortAbcCheck(x)}>
          {sortName}
        </button>
        <button class='sortFavButton' onClick={(x) => sortFavCheck(x)}>
          {favName}
        </button>
      </div>
      <div id='songList'>{displaySongs()}</div>
    </main>
  );
}
